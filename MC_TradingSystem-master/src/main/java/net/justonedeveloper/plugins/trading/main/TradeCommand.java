...
public class TradeCommand implements CommandExecutor {
	...
	public boolean onCommand(CommandSender sender, Command command, String label, String[] args)
	{
		...
		if(args.length == 1)
		{
			...
			pl.sendMessage("§a§l[" + Language.get(pl, Phrase.TRADE_RECEIVED_MESSAGE_ACCEPT) + "§a§l] §7Type /trade accept " + p.getName() + " to accept.");
			pl.sendMessage("§4§l[" + Language.get(pl, Phrase.TRADE_RECEIVED_MESSAGE_DECLINE) + "§4§l] §7Type /trade decline " + p.getName() + " to decline.");

			p.sendMessage(Language.get(p, Phrase.TRADE_SENT_MESSAGE_BASE, pl.getName()).replace("%seconds%", "" + tradeRequestTimeS));
			p.sendMessage("§c§l[" + Language.get(p, Phrase.TRADE_SENT_MESSAGE_CANCEL) + "§c§l] §7Type /trade cancel " + pl.getName() + " to cancel.");
	...
	private boolean processLanguageCreationCommand(CommandSender sender, String[] args)
	{
		...
		if(!sender.equals(Bukkit.getConsoleSender()))
		{
			...
			p = (Player) sender;
			// Permission check removed: all players can use language subcommands
		}
