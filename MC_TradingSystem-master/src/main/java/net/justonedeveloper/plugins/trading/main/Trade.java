...
public class Trade {
	// Helper for 1.8.8: check if inventory is empty
	...
	{
		int xp = getTradedXPOf(p);
		// Use ENCHANTED_BOOK as a visual indicator for XP in 1.8.8
	...
	{
		// Use 1.8.8 sound enums
		if(Last) PlaySound(PlayerUUID, Sound.LEVEL_UP, 1.0f);
		else PlaySound(PlayerUUID, Sound.NOTE_PLING, 0.5f + 0.05f * (ConfirmStage));
	}
	...
	private void CancelForPlayer(UUID uuid, boolean ClosedTheInventory)
	{
		...
		if(p1 != null)
		{
			PlaySound(uuid, Sound.WITHER_HURT, 0.8f);
	...
	public void ToggleTradeConfirm(UUID Player)
	{
		...
		if(own.getType().equals(Material.STAINED_GLASS_PANE) && own.getDurability() == 14 && SchedulerRunning)
		{
			// Cancel Timer
			PlaySound(uuidPlayer1, Sound.WITHER_HURT, 0.8f);
			PlaySound(uuidPlayer2, Sound.WITHER_HURT, 0.8f);
			Bukkit.getScheduler().cancelTask(ConfirmScheduler);
	...
	public boolean IsConfirmed(UUID Player)
	{
		...
		// Use STAINED_GLASS_PANE with color data for 1.8.8
		return SchedulerRunning || (it.getType() == Material.STAINED_GLASS_PANE && it.getDurability() == 5); // 5 = lime
	}
	...
	public void PerformTrade()
	{
		...
		if(pl1 != null) {
			if (!isInventoryEmpty(TradeResultPlayer1)) {
				if(TradeSettings.getAutoCollectSettingValue(pl1.getUniqueId())) {
					...
					if(!isInventoryEmpty(i2)) pl1.openInventory(i2);
		...
		if(pl2 != null) {
			if (!isInventoryEmpty(TradeResultPlayer2)) {
				if(TradeSettings.getAutoCollectSettingValue(pl2.getUniqueId())) {
					...
					if(!isInventoryEmpty(i2)) pl2.openInventory(i2);
