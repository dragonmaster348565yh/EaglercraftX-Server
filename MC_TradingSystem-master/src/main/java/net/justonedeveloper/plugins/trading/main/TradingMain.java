...
public final class TradingMain extends JavaPlugin {
	...
	* */
	...
	public void onEnable() {
		...
		main = this;
		// Check for unsupported server types
		String serverName = Bukkit.getServer().getName().toLowerCase();
		if (serverName.contains("bungee") || serverName.contains("waterfall") || serverName.contains("velocity")) {
			Bukkit.getLogger().warning("[TradingPlugin] This plugin is not compatible with BungeeCord/Waterfall/Velocity proxy servers. Use a dedicated proxy plugin for cross-server trading.");
		}
		Init();
	...
	{
	ItemStack it = new ItemStack(Material.STAINED_GLASS_PANE, 1, (byte) 14); // Red
	...
	{
	ItemStack it = new ItemStack(Material.STAINED_GLASS_PANE, 1, (byte) 5); // Lime
		ItemMeta m = it.getItemMeta();
		assert m != null;
	...
	{
	ItemStack it = new ItemStack(Material.STAINED_GLASS_PANE, 1, (byte) 3); // Light Blue
		ItemMeta m = it.getItemMeta();
		assert m != null;
	...
	{
	ItemStack it = new ItemStack(Material.STAINED_GLASS_PANE, 1, (byte) 13); // Green
		ItemMeta m = it.getItemMeta();
		assert m != null;
		m.setDisplayName(Language.get(uuid, Phrase.XP_TRADING_GUI_ADD_XP_NAME));
	...
	{
	ItemStack it = new ItemStack(Material.STAINED_GLASS_PANE, 1, (byte) 1); // Orange
		ItemMeta m = it.getItemMeta();
		assert m != null;
	...
	{
	ItemStack it = new ItemStack(Material.GLASS_BOTTLE, 1); // No XP bottle in 1.8.8
		ItemMeta m = it.getItemMeta();
		assert m != null;
	...
	public void Init()
	{
	ItemStack it = new ItemStack(Material.STAINED_GLASS_PANE, 1, (byte) 7); // Gray
		ItemMeta m = it.getItemMeta();
		assert m != null;
		m.setDisplayName("§f ");
		it.setItemMeta(m);
